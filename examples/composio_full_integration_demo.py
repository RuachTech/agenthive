...
async def demo_unified_integration():
    ...
    for mode in modes:
        ...
        try:
            ...
            if tools:
                sample_tool = tools[0]
                print(f"   Executing sample tool: {sample_tool.name} (from {sample_tool.source})")
...
async def demo_agent_workflow():
    ...
    try:
        ...
        if setup_result.get("mcp_setup", {}).get("success"):
            ...
            print(f"   MCP: {mcp_setup['tools_count']} tools, categories: {mcp_setup['categories']}")
        ...
        tasks = [
            {
                ...
                "args": {"name": "agent-created-repo", "description": "Repository created by AI agent"}
            },
            {
...
async def demo_custom_tools():
    ...
    try:
        ...
        tool_slug = client.create_custom_tool(
            ...
            input_params={"n": {"type": "integer", "description": "The position in Fibonacci sequence"}},

...
async def setup_demo_environment() -> tuple[AgentFactory, list[str]]:
    ...
    logger.info(
        f"Demo environment setup complete with {len(agents_created)} agents: {agents_created}"
    )
    return agent_factory, agents_created


async def setup_mock_environment() -> tuple[AgentFactory, list[str]]:
...
async def demo_session_management() -> SessionManager:
    ...
    try:
        ...
        success = await session_manager.update_session_state(
            "demo_session_001", retrieved_session.state
        )
        assert success
...
async def demo_graph_creation(
    agent_factory: AgentFactory, available_agents: list[str]
    ...
    try:
        ...
        for agent_name in available_agents:
            ...
            try:
                ...
                logger.info(
                    f"  Graph structure validation: {validation_result['valid']}"
                )
...
async def demo_direct_chat_execution(
    graph_factory: DirectChatGraphFactory, available_agents: list[str]
) -> None:
...
async def demo_mock_execution() -> None:
...
async def demo_error_handling(graph_factory: DirectChatGraphFactory) -> None:
...
async def main() -> None:
    ...
    if not openai_key and not anthropic_key:
        ...
        logger.info(
            "To test with real models, set OPENAI_API_KEY or ANTHROPIC_API_KEY environment variables"
        )

...
async def demo_agent_factory() -> None:
    ...
    try:
        ...
        print(f"   Agent node creation failed (expected without proper model setup): {e}")
...
async def demo_error_recovery() -> None:
    ...
    print(f"   Recovery info in scratchpad: {'test_agent_recovery' in timeout_state['scratchpad']}")
    
    # 2. Tool failure recovery
    print("\n2. Tool Failure Recovery...")
    tool_error = Exception("Tool connection failed")
    tool_state = await recovery.handle_tool_failure(error_state, "test_agent", "demo_tool", tool_error)
    print(f"   Tool failure recorded: {tool_state['errors'][-1]['type'] == 'tool_failure'}")
    
    # 3. Validation failure recovery
    print("\n3. Validation Failure Recovery...")
    validation_errors = ["Missing required capability", "Model unavailable"]
    validation_state = await recovery.handle_validation_failure(error_state, "test_agent", validation_errors)
    print(f"   Validation errors recorded: {len(validation_state['errors'][-1]['validation_errors'])}")

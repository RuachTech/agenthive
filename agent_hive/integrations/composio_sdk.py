...
try:
    ...
    from composio.types import ExecuteRequestFn  # type: ignore[import-not-found]
...
class ComposioSDKClient:
    ...
    async def authorize_toolkit(
        ...
        try:
            ...
            logger.info(
                "Authorization initiated for user %s, toolkit %s", user_id, toolkit
            )
            return connection

        except Exception as e:
            logger.error(
                "Failed to authorize toolkit %s for user %s: %s", toolkit, user_id, e
            )
            raise ComposioAuthenticationError(f"Authorization failed: {e}")

    async def wait_for_connection(
        self, connection: ComposioConnection, timeout: int = 300
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                "Connection established for user %s, toolkit %s",
                connection.user_id,
                connection.toolkit,
            )
            return True
    ...
    async def get_tools(
        ...
        try:
            ...
            composio_tools = self.composio.tools.get(
                user_id=user_id, toolkits=toolkits, tools=tools
            )
    ...
    def create_custom_tool(
        ...
        try:
            if toolkit:
                ...
                def custom_tool_impl(request: Any) -> Any:
...
class ComposioAgentIntegration:
            ...
            try:
                ...
                if connection.redirect_url:
                    setup_results["authorization_urls"].append(
                        {"toolkit": toolkit, "url": connection.redirect_url}
                    )
                ...
                logger.error(
                    "Failed to setup toolkit %s for agent %s: %s",
                    toolkit,
                    agent_type,
                    e,
                )
                setup_results["connections"].append(
                    {"toolkit": toolkit, "status": "failed", "error": str(e)}
                )
    ...
    async def execute_agent_tool(
        self, user_id: str, agent_type: str, tool_slug: str, arguments: Dict[str, Any]
    ) -> ComposioExecutionResult:
        """Execute a tool on behalf of an agent."""
        logger.info("Executing tool %s for agent %s", tool_slug, agent_type)

        return await self.composio.execute_tool(
            user_id=user_id, tool_slug=tool_slug, arguments=arguments
        )
    ...
    async def get_agent_status(self, user_id: str, agent_type: str) -> Dict[str, Any]:
        ...
        for toolkit in toolkits:
            toolkit_connections = [
                conn for conn in connections if conn.toolkit == toolkit
            ]

            if toolkit_connections:
                latest_connection = max(
                    toolkit_connections, key=lambda x: x.connection_id
                )

...
class UnifiedIntegrationClient:
    ...
    def _initialize_clients(
        self, composio_api_key: Optional[str], mcp_config_path: Optional[str]
    ) -> None:
        ...
        if (
            ...
            try:
                ...
                self.composio_integration = ComposioAgentIntegration(
    ...
    async def authorize_toolkit(
        self, user_id: str, toolkit: str, redirect_url: Optional[str] = None
    ) -> Dict[str, Any]:
        """Authorize a toolkit for a user (Composio SDK only)."""
        if not self.composio_client:
            raise ComposioIntegrationError(
                "Composio SDK not available for authorization"
            )
    ...
    async def get_tools_for_agent(
        self, user_id: str, agent_type: str, capabilities: Optional[List[str]] = None
    ) -> List[UnifiedTool]:
        ...
        if self.mcp_client:
            try:
                mcp_tools = await self.mcp_client.get_tools_for_agent(
        ...
        logger.info(
            "Retrieved %d unified tools for agent %s", len(all_tools), agent_type
        )
        return all_tools
    ...
    async def execute_tool(
        ...
        if source == "mcp" and self.mcp_client:
    ...
    async def _execute_composio_tool(
        self, user_id: str, tool_slug: str, arguments: Dict[str, Any]
    ) -> UnifiedExecutionResult:
    ...
    def _determine_execution_source(
        self, tool_slug: str, preferred_source: Optional[str]
    ) -> str:
            ...
            try:
                ...
                status["composio_status"] = {
                    ...
                    "active_connections": len(
                        [c for c in connections if c.status == "active"]
                    ),
                }
            except Exception as e:
    ...
    async def setup_agent_integrations(
        self, user_id: str, agent_type: str, auto_authorize: bool = False
    ) -> Dict[str, Any]:
...
async def get_unified_client(
    ...
    if _unified_client is None:
        _unified_client = UnifiedIntegrationClient(
            mode, composio_api_key, mcp_config_path
        )

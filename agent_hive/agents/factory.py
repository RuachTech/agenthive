...
class ErrorRecoveryStrategy:
    ...
    async def handle_tool_failure(
        state: AgentState, agent_name: str, tool_name: str, error: Exception
    ) -> AgentState:
    ...
    async def handle_validation_failure(
        state: AgentState, agent_name: str, validation_errors: List[str]
    ) -> AgentState:
...
class AgentNodeWrapper:
    ...
    async def __call__(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            return await self.recovery_strategy.handle_model_timeout(
    ...
    async def _validate_execution_requirements(self, state: AgentState) -> None:
        ...
        if "vision" in self.capabilities.model_requirements:
            if not state.get("multimodal_content"):
                validation_errors.append(
                    "Vision capability required but no multimodal content provided"
                )
        ...
        for required_tool in self.capabilities.required_tools:
            if required_tool not in available_tool_names:
                validation_errors.append(
                    f"Required tool '{required_tool}' not available"
                )

        if validation_errors:
            raise AgentValidationError(
                self.agent_name, "validation_failed", "; ".join(validation_errors)
            )
        ...
        if state.get("scratchpad"):
            ...
            for key, value in state["scratchpad"].items():
                if not key.startswith(
                    f"{self.agent_name}_"
                ):  # Don't include own previous work
...
class AgentFactory:
    ...
    async def create_agent_node(
        ...
        if agent_name not in self.agent_configs:
            raise ValueError(
                f"Agent '{agent_name}' not registered. Call register_agent_config first."
            )
    ...
    def load_agent_configurations(self, config_dir: Path) -> None:
        ...
        for config_file in config_dir.glob("*.json"):
            try:
                ...
                capabilities = AgentCapabilities(
                    required_capabilities=capabilities_data.get(
                        "required_capabilities", []
                    ),
                    optional_capabilities=capabilities_data.get(
                ...
                logger.error(
                    "Failed to load agent config from %s: %s", config_file, str(e)
                )

...
class TestMockModel:
    ...
    @pytest.mark.asyncio
    async def test_mock_model_unavailable_generate(
    ...
    @pytest.mark.asyncio
    async def test_mock_model_unavailable_stream(
        self, openai_config: ModelConfig
    ) -> None:
...
class TestModelFactory:
    ...
    async def test_get_available_model(self, openai_config: ModelConfig) -> None:
        ...
        with patch("agent_hive.core.models.ChatOpenAI"):
            ...
            with patch.object(
                factory._model_instances["test-model"],
                "is_available",
    ...
    @pytest.mark.asyncio
    async def test_get_model_with_fallback(
        self, openai_config: ModelConfig, anthropic_config: ModelConfig
    ) -> None:
        """Test getting model with fallback when primary unavailable."""
        factory = ModelFactory()

        with (
            patch("agent_hive.core.models.ChatOpenAI"),
            ...
            factory.register_model(
                "with-fallback", openai_config, fallbacks=["fallback"]
            )

            # Mock primary as unavailable, fallback as available
            with (
                patch.object(
                    factory._model_instances["with-fallback"],
                    "is_available",
                    new_callable=AsyncMock,
                ) as mock_primary,
                patch.object(
                    factory._model_instances["fallback"],
                    "is_available",
                    new_callable=AsyncMock,
                ) as mock_fallback,
            ):
    ...
    async def test_check_all_models(
        ...
        with (
            ...
            with (
                patch.object(
                    factory._model_instances["model1"],
                    "is_available",
                    new_callable=AsyncMock,
                ) as mock1,
                patch.object(
                    factory._model_instances["model2"],
                    "is_available",
                    new_callable=AsyncMock,
                ) as mock2,
            ):

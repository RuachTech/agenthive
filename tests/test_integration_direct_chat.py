...
class TestDirectChatIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_workflow_without_models(self) -> None:
        ...
        success = await session_manager.update_session_state(
            "integration_test_001", retrieved_session.state
        )
        ...
        validation_result = graph_factory.validate_graph_structure(
            mock_graph, "integration_test_agent"
        )
    ...
    @pytest.mark.asyncio
    async def test_multiple_sessions_and_agents(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_session_expiry_handling(self) -> None:
        ...
        try:
            ...
            await short_timeout_manager.create_session(
                session_id="expiry_test", user_id="test_user", mode="direct"
            )
    ...
    @pytest.mark.asyncio
    async def test_state_persistence_structure(self) -> None:
